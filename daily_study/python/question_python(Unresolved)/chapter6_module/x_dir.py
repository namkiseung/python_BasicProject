    ...
    for dirpath, dirnames, filenames in os.walk(workDir):        
        ...
        for dirname in dirnames:                        
            ...
            if int(trans_time) >= 12:
                #tt=int(trans_time) - 12
                ifafter_time_text = ' 오후'                
            a += str(lasttime)[:10]+ifafter_time_text+str(lasttime)[10:19]+'  <DIR>'+str(getsize(dirname))+dirname+'\n'
        ...
        for filename in filenames:
            ...
            if int(trans_time) >= 12:
                ifafter_time_text = ' 오후'
                #tt=int(trans_time) - 12
            b += str(lasttime)[:10]+ifafter_time_text+str(lasttime)[10:19]+'\t'+''+str(getsize(filename))+filename+'\n'
          
    real_result=str(out)+str(a)+str(b)+str()+str(c)#str(unicode('{}'.format(c), 'utf-8'))
    return real_result#out,a,b,c
...
def direct_size():
    ...
    for root, dirs, files in os.walk(workDir):
        ...
        #print '총크기',all_size
    x ='\t',str(file_size)+'개 파일','\t\t',a.st_size,' 바이트'
    y ='\t',str(folder_size)+'개 디렉터리','\t\t',str_count_dat(a.st_mtime),'바이트 남음'    
    return x,y
