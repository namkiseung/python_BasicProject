...
def user_add(sitename='', snsauth='', url='', userid='', userpw=''):
    ms = Site_info(sitename, snsauth, url, userid, userpw)
    db.session.add(ms)
    db.session.commit()
    pass
def user_update(sitename='', snsauth='', url='', userid='', userpw=''):
    ms = Site_info.query.filter(Site_info.userid == userid).first()
...
def user_del(sitename):
    ms = Site_info.query.filter(Site_info.sitename == sitename).first()
...
def index():
    ...
    return render_template('index.html', sitelist = sitelist, ip = get_ipaddr())

@app.route("/login", methods=['GET', 'POST'])
def loginpage():
    return render_template('login.html', ip = get_ipaddr())
...
def view_route(sitename, _userid):
    ...
    if chk_site is not None:
        ...
        if chk_userid is not None:
            res_pw=chk_userid.userpw
            return json.dumps(["ID : {}".format(chk_userid.userid), "PW : {}(len {})".format(res_pw.upper(), len(res_pw))])
